...
interface LinkProps {
  ...
  text: string;
  variant?: 'primary' | 'secondary' | 'success' | 'danger' | 'warning' | 'gray' | 'blue-light';
  underline?: boolean;
...
const Link: React.FC<LinkProps> = ({
  ...
  // Opacity handling
  const opacityClass = opacity ? `text-gray-500/${opacity} dark:text-gray-400/${opacity}` : '';

  // Hover effects
  const hoverClass = hoverEffect
    ? `hover:text-gray-500/${opacity || 100} dark:hover:text-gray-400/${opacity || 100}`
    : '';
